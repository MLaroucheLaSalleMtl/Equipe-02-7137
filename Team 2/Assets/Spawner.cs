...
{
    [SerializeField] MonsterHandler mHandler;
    ...
    void Spawn()
    {
        ...
        if(Vector3.Distance(player.transform.position,transform.position)<playerRange)
        {
            ...
            mHandler.SpawnMonster(type[monster], new Position(spawnX, (int)transform.position.y, spawnZ), type[monster].ToString(), 1, 5, 5);
        }
        Invoke("Spawn", time);
    }
